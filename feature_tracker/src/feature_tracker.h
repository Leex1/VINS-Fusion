...
class FeatureTracker
{
  public:
    ...
    cv::Mat mask;
    cv::Mat fisheye_mask;
    cv::Mat prev_img, cur_img, forw_img;// 上上帧图像 | 光流跟踪的上一帧 | 当前帧
    vector<cv::Point2f> n_pts;// 每一帧中提取的新特征点
    vector<cv::Point2f> prev_pts, cur_pts, forw_pts;// 对应帧的特征点
    ...
    static int n_id;//用来作为特征点id，在updateID函数中 每检测到一个新的特征点，就将++n_id作为该特征点
};
